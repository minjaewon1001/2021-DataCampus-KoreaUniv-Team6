...
VGGNet은 (3x3) 필터를 사용합니다. 이론적으로는 (5x5) 필터를 한 번 사용하는 것과 동일한 성능을 보여야하지만, 연구진이 테스트한 결과 (3x3) 필터를 여러 번 사용하는 방법이 더 좋은 성능을 보였습니다. 그 이유로는 크게 두 가지 정도가 있습니다. 
...
### 모델의 단점
파라미터 개수가 너무 많다. 특히 fully-connected layer에서의 파라미터는 너무 많다.
...
그 결과, VGG를 기본 모델로 하는 수 많은 발전된 모델이 등장한다.
